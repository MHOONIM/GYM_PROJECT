    ...
    def network_training(self, experience_size):
        ...
        for k in range(self.epoch):
            ...
            with tf.GradientTape() as tape_1:
                ...
                # current_policy_tensor = tf.math.log(current_policy_tensor)
                # Probability Ratio
                prob_ratio = tf.divide(current_policy_tensor, old_policy_tensor)
                # prob_ratio = tf.exp(tf.subtract(current_policy_tensor, old_policy_tensor))
                # Unclipped
                un_clipped = tf.multiply(prob_ratio, a_hat_tensor)
                # Clipped
                clipped = tf.clip_by_value(prob_ratio,
                                           1 - self.epsilon,
                                           1 + self.epsilon)
                clipped = tf.multiply(clipped, a_hat_tensor)
                # Cost Function
    ...
    def agent_training(self, episode):
        for i in range(episode):
            ...
            while not self.terminated:
                ...
                # selected_action_policy = tf.math.log(selected_action_policy)
